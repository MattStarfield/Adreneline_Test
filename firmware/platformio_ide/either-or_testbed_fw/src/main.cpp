...
// Flag: indicator that an event has occured; handle event and reset flag
  ...
  //----------------------------------------------------------------------------
    ...
    //uint8_t uvLedPwm = 0;
    uint16_t uvLedStrobeOnTime_ms = 0;
...
//==============================================================================
  ...
  //----------------------------------------------------------------------------
    ...
    EasyLed uvLedStrobe(PIN_UV_LED_STROBE);
...
//=========================================================================//
  ...
  void setup()
  {
    ...
    // Options: OUTPUT, INPUT, INPUT_PULLUP
      ...
      // EasyLed Object: uiLed                    // 13 // SPI-SCK; OB-LED //
      // NeoPixel Object: pixels                  // 12 // SPI-MISO  // setup in pixel_config.h
      // EasyLed Object: uvLedStrobe              // 11 // SPI-MOSI; PWM //
      // EasyBuzzer Object: EasyBuzzer            // 10 // PWM        // UI Alert Buzzer
      // EasyLed Object: uiLed                   //  9 // PWM        //
      // pinMode();                              //  8 //            // not on Feather
      // pinMode();                              //  7 //            // not on Feather
      pinMode(PIN_FAN_PWM, OUTPUT);              //  6 // PWM        //
      pinMode(PIN_FAN_MODE, INPUT_PULLUP);       //  5 // PWM        //
      // pinMode();                              //  4 //           // not on Feather
      pinMode(PIN_FAN_TACH, INPUT_PULLUP);       //  3 // INT1      // Active LOW
      // EasyButton Object: uiButton              //  2 // INT0      // UI Button; setup with EasyButton lib
      // pinMode();                               //  0 // RX0 (avoid use) //
      // pinMode();                               //  1 // TX1        //

      pinMode(PIN_POT, INPUT);                    // A0 // Analog In  //
...
*/
    ...
    //----------------------------------------------------------------------------
      digitalWrite(PIN_UV_LED_STROBE, LOW);
...
*/
        ...
        //----------------------------------------------------------------------------
            ...
            )
          {
            ...
            potValAvg = constrain(potValAvg, POT_TOLERANCE, (1023-POT_TOLERANCE));       // forces pot val to known range to account for pot variations

            //fanSpeedPwm = map(potValAvg, 0, 900, FAN_PWM_MIN, FAN_PWM_MAX);   // map potVal range to fanSpeedPwm range
            uvLedStrobeOnTime_ms = map(potValAvg, POT_TOLERANCE, (1023-POT_TOLERANCE), 0, 1000);   // map potAvg range to strobeOnTime values from 0 to 1 sec

            uvLedStrobeOnTime_ms = constrain(uvLedStrobeOnTime_ms, 0, 1000);
        ...
        // Update Fan Mode (High or Low)
        //----------------------------------------------------------------------------
        // Fan Mode pin set with jumper
        // if HIGH, fan runs at MAX speed
        // if LOW, fan runs at MIN speed

          if(digitalRead(PIN_FAN_MODE))
          {
      ...
      //----------------------------------------------------------------------------
      {
        ...
        // NOTE: This state handles all actions that should be performed on a "warm reboot" command
          ...
          //----------------------------------------------------------------------------
            ...
            uvLedState = false;
            Debug.print(DBG_DEBUG, F("[D] * UV LEDs:\t OFF") );
...
*/
      ...
      //----------------------------------------------------------------------------
      {
        ...
        fanState = true;
        // fanSpeedPwm set by jumper state in Handle Polled Inputs
        Debug.print(DBG_DEBUG, F("[D] * Fan:\t ON") );

        uvLedState = true;
        // uvLedStrobe timing set by Pot value in Handle Polled Inputs
        Debug.print(DBG_DEBUG, F("[D] * UV LEDs:\t ON") );

        potValUpdateFlag = true;    // force initial strobe pattern setting in "Update Actuator Outputs"
      ...
      //----------------------------------------------------------------------------
      {
        ...
        // Safety: Monitor fanIsRunning allow UV LEDs to be ON ...
          ...
          {
            // Fan & UV LEDs turned off in FSM_SYS_INIT_ERROR
        ...
        // Press button to turn off UV LEDs and Fan
      ...
      //----------------------------------------------------------------------------
      {
        // Turn off fan and UV LEDs when leaving stage
          fanState = false;
          Debug.print(DBG_DEBUG, F("[D] * Fan:\t OFF") );
...
*/
    ...
    //----------------------------------------------------------------------------
      ...
      //----------------------------------------------------------------------------
        ...
        // UV LED Module
        //----------------------------------------------------------------------------
        //if(    uvLedState         // UV LEDs are set to be ON
        //    && fanIsRunning       // fanIsRunning is determined from fanTachCount in "Handle ISRs"
        //    && potValUpdateFlag   // only run once when potVal is updated
        //)

        if(    potValUpdateFlag     // only run ONCE when potVal is updated to set blink/strobe pattern
            && fanIsRunning         // make sure fan is running before updating UV LED strobe
            && uvLedState
          )
        {
          potValUpdateFlag = false;   // reset flag

          // set stobing pattern for UV LEDs here, uiLed.update() happens below
            //led.blink(             onFor_ms,                   offFor_ms,  cycles,  pauseFor_ms,  numOfSequeces,  onFinished_cb() );
            uiLed.blink( uvLedStrobeOnTime_ms, (1000-uvLedStrobeOnTime_ms),       2,            0,           0,                NULL);

            Debug.print(DBG_DEBUG,    F("[D] uvLedStrobeOnTime_ms: %d"), uvLedStrobeOnTime_ms );
        }
